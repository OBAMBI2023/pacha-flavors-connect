...
function ConfirmationPage() {
  ...
  const fallbackSlug =
    typeof window === "undefined"
      ? ""
      : (window.localStorage.getItem(STORAGE_RESTAURANT_SLUG) ?? "");
  ...
  // Foreground half of the notification engine ("Realtime pour les mises à
  // jour instantanées dans l'application ouverte") -- complements the 10s
  // poll above rather than replacing it: an instant toast on the same
  // status change the poll would otherwise only surface up to 10s later,
  // never the only way this page learns of an update.
  useNotificationEngine({ userId: null, visitorId: getOrCreateVisitorId() });
  ...
  useMetaPixelPurchase(
    order && !isCancelled
      ? { id: order.id, total_amount: order.total_amount, currency: order.currency }
      : null,
  );
        ...
        <main className="mx-auto max-w-2xl px-4 py-6 sm:px-6">
          ...
          {!order ? (
            <p className="mt-8 text-center text-sm text-muted-foreground">
              {query.isLoading ? "Chargement..." : "Commande introuvable."}
            </p>
          ) : (
            <>
              <div
                className={`mt-5 flex flex-col items-center gap-1 rounded-3xl p-6 text-center ${isCancelled ? "border border-destructive/20 bg-destructive/5" : "border border-border bg-card shadow-sm"}`}
              >
                {isCancelled ? (
                ...
                <p className="text-sm text-muted-foreground">
                  {formatOrderNumber(order)} · {new Date(order.created_at).toLocaleString("fr-FR")}
                </p>
                {!isCancelled && order.estimated_preparation_minutes !== null && (
                  <p className="mt-2 flex items-center gap-1.5 text-sm font-medium text-foreground">
                    <Clock className="h-4 w-4 shrink-0 text-primary" /> Préparation estimée :
                    environ {order.estimated_preparation_minutes} min
                  </p>
                )}
              </div>
              ...
              <div className="mt-4 space-y-4 rounded-3xl border border-border bg-card p-5">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-foreground">
                      {order.fulfillment_type === "delivery" ? "Livraison" : "Retrait sur place"}
                    </p>
                  </div>
                  <div>
                ...
                <div className="space-y-1.5 border-t border-border pt-4">
                  ...
                  {order.fulfillment_type === "delivery" && order.delivery_fee_amount > 0 && (
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-foreground">
                        {formatMoney(order.delivery_fee_amount, order.currency)}
                      </span>
                    </div>
                  )}
