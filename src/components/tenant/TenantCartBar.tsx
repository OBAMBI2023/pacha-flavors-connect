    ...
    <div className="fixed inset-x-0 bottom-[calc(76px+env(safe-area-inset-bottom))] z-40 px-3 pb-2 pt-2 md:hidden">
      ...
      >
        <span className="relative grid h-9 w-9 shrink-0 place-items-center rounded-full bg-primary-foreground/15">
          ...
          <span className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full bg-foreground px-1 text-[0.65rem] font-semibold text-background">
            {count}
          </span>
        </span>
        <span className="min-w-0 flex-1 text-left text-xs leading-tight text-primary-foreground/80">
          {count} article{count > 1 ? "s" : ""} dans le panier
          <br />
          <span className="text-sm font-bold text-primary-foreground">Total {subtotalLabel}</span>
        </span>
        <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-foreground px-4 py-2.5 text-xs font-semibold text-background">
