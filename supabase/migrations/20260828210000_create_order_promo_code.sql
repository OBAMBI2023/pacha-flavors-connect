...
begin
  ...
  -- Early, read-only customer lookup (by normalized phone) so a promo code's
  -- targeting/per-customer-usage rules can be checked before pricing
  -- finishes. The full find-or-create upsert still happens later, unchanged,
  -- once v_total is known.
  ...
  -- Promo code: resolved after per-line offers/product-promotions so its
  -- discount is computed off the already-discounted subtotal, and folded
  -- into the same v_discount_amount accumulator they use. A free_delivery
  -- code waives delivery exactly like a free_delivery product promotion.
