-- orders.subtotal_amount previously stored the POST-discount sum (unit
-- prices were already discounted before being summed), while discount_amount
-- was tracked separately just for display. That made the admin breakdown
-- (Sous-total + Frais de livraison - Réduction = Total) not reconcile
-- whenever a promo/offer was active -- confirmed live: an order with a 2250
-- offer price (vs 2500 list price) x2 produced subtotal=4500,
-- discount=500, total=4500, where 4500-500=4000 != 4500.
--
-- subtotal_amount now stores the PRE-discount sum instead, so
-- subtotal - discount + delivery_fee == total (matches the invoice-style
-- breakdown already shown in OrderDetailSheet). total_amount itself is
-- numerically unchanged: order_items.unit_price_snapshot/line_total (what
-- was actually charged) are untouched, only the order-level subtotal
-- aggregate's meaning changes.
