-- The cart form no longer collects "Commune / quartier" for delivery orders.
-- create_order still accepts p_delivery_commune (kept optional, stored as
-- before) but must stop requiring it -- only the delivery address remains
-- mandatory for a delivery order. No column, RLS, or policy changes.

CREATE OR REPLACE FUNCTION public.create_order(p_slug text, p_fulfillment_type order_fulfillment_type, p_customer_name text, p_customer_phone text, p_items jsonb, p_delivery_commune text DEFAULT NULL::text, p_delivery_address text DEFAULT NULL::text, p_delivery_instructions text DEFAULT NULL::text, p_customer_notes text DEFAULT NULL::text, p_order_source text DEFAULT 'direct'::text, p_source_metadata jsonb DEFAULT '{}'::jsonb, p_payment_method text DEFAULT 'cash'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
