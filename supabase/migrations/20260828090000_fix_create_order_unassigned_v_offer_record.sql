-- Bug report: "Impossible de créer la commande" on every checkout.
--
-- Root cause: v_offer is a plain `record` (no fixed composite type), and
-- its SELECT INTO only ran inside `if p_offer_id is not null`. For a
-- normal order (p_offer_id null, which is every order placed from the
-- storefront today), that SELECT never executed, so v_offer had no tuple
-- structure at all -- the very next `v_offer.id is not null` reference in
-- the items loop then raised "record v_offer is not assigned yet"
-- unconditionally, for every single order.
--
-- Fix: run the SELECT unconditionally. `o.id = p_offer_id` naturally
-- matches zero rows when p_offer_id is null, so v_offer ends up as a
-- properly-structured all-NULL record instead of an unassigned one --
-- exactly like v_promo (a typed variable) already behaves. Only the
-- "offer not found" exception stays conditional on p_offer_id being set.
