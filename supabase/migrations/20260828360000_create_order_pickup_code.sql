-- Generates the 2-digit pickup code at order-creation time for delivery
-- orders only. Single point of truth, no regeneration later, no change to
-- the function's parameter list (same signature -> replaces in place).
...
begin
  ...
  -- 2-digit pickup code (00-99), generated once, only for delivery orders.
  -- Never regenerated afterwards -- verify_pickup_code checks against this
  -- exact stored value for the life of the order.
  ...
  insert into public.orders (
    ...
    restaurant_lat_snapshot, restaurant_lng_snapshot, cutlery_requested, pickup_code
  ) values (
    ...
    v_restaurant.lat, v_restaurant.lng, coalesce(p_cutlery_requested, false), v_pickup_code
  )
