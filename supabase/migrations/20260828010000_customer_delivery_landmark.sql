-- Adds a dedicated "point de repère" (landmark) field, distinct from the
-- free-form delivery_instructions note, so an order snapshot carries enough
-- to hand off to a driver or a third-party delivery app: GPS + readable
-- address + neighborhood/commune/city + a landmark + delivery instructions.
-- The Google Maps link itself is derived from delivery_latitude/longitude
-- at display time (client and admin) rather than stored, since it's fully
-- reproducible from data already on the row.

alter table public.orders
  add column if not exists delivery_landmark text;

-- Adding a trailing param changes create_order's signature, so
-- CREATE OR REPLACE below creates a new overload rather than replacing the
-- old one -- drop the old 16-param signature first so only one create_order
-- exists (an ambiguous-call error otherwise awaits any caller that doesn't
-- pass every optional param).
drop function if exists public.create_order(text, order_fulfillment_type, text, text, jsonb, text, text, text, text, text, jsonb, text, double precision, double precision, text, text);

-- Additive redefinition of create_order: same body as before, plus one new
-- optional param (appended at the end so existing named-argument callers
-- are unaffected) that is snapshotted onto the new order row.
