-- Wires up the existing (previously unused-by-the-app) restaurant_products
-- .prep_time_minutes column as the customer-facing "preparation time" field,
-- tightens its allowed range to 1-240 min (was >= 0, allowing 0 and any
-- upper bound), and snapshots a single per-order estimate onto the order at
-- creation time so the value shown to the customer never drifts if a dish's
-- prep time is edited later.

alter table public.restaurant_products
  drop constraint if exists restaurant_products_prep_time_minutes_check;

alter table public.restaurant_products
  add constraint restaurant_products_prep_time_minutes_check
...
-- Same signature as before (the estimate is computed server-side from the
-- ordered products, never supplied by the caller) -- only the body changes,
-- so this is a plain CREATE OR REPLACE, no overload/drop needed.
...
declare
  ...
  -- Highest prep_time_minutes among the ordered products that actually have
  -- one set; stays NULL if none of them do (never a fabricated estimate).
