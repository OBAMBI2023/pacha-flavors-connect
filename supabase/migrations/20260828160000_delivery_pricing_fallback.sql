-- The GPS-unavailable delivery fallback was silently reusing
-- restaurant_settings.delivery_fee (the tenant's separate flat-fee
-- setting, currently 0.00 for every real tenant -- confirmed live) --
-- meaning a customer whose address had no coordinates got FREE delivery
-- instead of a real fee. New dedicated, tenant-configurable column with a
-- 1500 FCFA default, used only when the distance-based calculation can't
-- run (never touches the distance-based path, which stays untouched).

alter table public.restaurant_settings
  add column delivery_fee_fallback numeric not null default 1500;

-- get_public_menu additive change: exposes the fallback amount so the
-- storefront preview shows the real configured value, not a hardcoded
-- guess that could drift from the backend.
...
begin
  ...
  -- point. Falls back to the tenant's dedicated delivery_fee_fallback
  -- (default 1500, never the unrelated flat delivery_fee, never 0) when
  -- either endpoint's coordinates are unavailable, and a free_delivery
