-- Adds the tenant's weekly opening hours (tenant_business_hours, currently
-- only readable by authenticated restaurant members) to get_public_menu's
-- payload as `business_hours`, so the public storefront can render a real
-- schema.org openingHoursSpecification instead of omitting hours entirely.
-- Opening hours are inherently public information a restaurant wants
-- customers to see -- no privacy concern, same security-definer pattern as
-- every other field already exposed by this RPC.
