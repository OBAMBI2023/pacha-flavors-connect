-- Persists everything captured at checkout as real, individually
-- queryable/displayable columns on the order itself (a true snapshot --
-- never re-derived from a customer/tenant profile later), instead of the
-- free-text-field workaround from the previous pass (allergies crammed
-- into customer_notes, recipient info prepended into delivery_instructions
-- text). delivery_address already IS the "effective delivery address" --
-- no separate column for that: it was already set to the recipient's
-- address when ordering for someone else in the prior migration, so
-- duplicating it under a new name would just be two columns that could
-- drift. customer_profile_address is new: the orderer's own confirmed
-- checkout address, kept distinct from delivery_address precisely so the
-- two can differ.
--
-- delivery_distance_km/delivery_fee_calculation_method: create_order
-- already computes the distance (haversine_km) and decides between the
-- distance-based and fallback fee -- it just never wrote either fact to
-- the order row. Fixed here, no pricing logic changed.
--
-- driver_note is a distinct column from delivery_instructions per the
-- spec's field list, but this app's checkout has exactly one free-text
-- "Instructions pour le livreur" input -- there is no second, separately
-- authored "consigne" the customer types. Both columns are populated from
-- that same single input rather than inventing a second UI field with an
-- unclear distinction from the first.
...
begin
  ...
  insert into public.orders (
    ...
    restaurant_lat_snapshot, restaurant_lng_snapshot
  ) values (
    ...
    nullif(btrim(coalesce(p_allergy_information, '')), ''), nullif(btrim(coalesce(p_driver_note, '')), ''),
    ...
    v_restaurant.lat, v_restaurant.lng
  )
...
drop function if exists public.create_order(text, order_fulfillment_type, text, text, jsonb, text, text, text, text, text, jsonb, text, double precision, double precision, text, text, text, uuid, text);
