-- Scheduled orders: lets a customer order ahead for the next open window
-- when the restaurant is currently closed, instead of being fully blocked.
-- Reuses get_restaurant_availability(id, p_now) exactly as-is -- it already
-- accepts an arbitrary point in time, so validating a future slot needs no
-- new schedule/exception logic, just calling it with p_scheduled_for
-- instead of the implicit now(). "Commande programmée" is identified purely
-- by orders.scheduled_for is not null -- no new order_status, no new table.

alter table public.orders add column scheduled_for timestamptz;

-- create_order: signature unchanged except one new trailing default
-- parameter (safe append, no overload ambiguity) and two edits to the body:
-- (1) the is_open check branches on whether a slot was requested, and
-- (2) scheduled_for is persisted + echoed back + mentioned in the kitchen
-- notification. Everything else is the exact live body, untouched.
create or replace function public.create_order(
  ...
  p_items jsonb,
  p_delivery_commune text default null::text,
  ...
  p_order_source text default 'direct'::text,
  ...
  p_delivery_neighborhood text default null::text,
  ...
  p_is_for_someone_else boolean default false,
  ...
  p_recipient_city text default null::text,
  p_recipient_neighborhood text default null::text,
  p_recipient_landmark text default null::text,
  p_recipient_additional_info text default null::text,
  p_allergy_information text default null::text,
  p_driver_note text default null::text,
  p_customer_profile_address text default null::text,
  p_cutlery_requested boolean default false,
...
declare
  ...
  v_promo_code_row record;
  v_promo_discount numeric := 0;
  v_promo_waives_delivery boolean := false;
  v_promo_code_id_to_store uuid;
  v_promo_code_snapshot text;
  v_pickup_code text;
begin
  ...
  if v_phone_normalized is not null then
    select id into v_promo_customer_id
  ...
  if p_scheduled_for is not null then
    if p_scheduled_for <= now() then
      raise exception 'Le créneau sélectionné est déjà passé.';
    end if;
    v_availability := public.get_restaurant_availability(v_restaurant.id, p_scheduled_for);
    if not coalesce((v_availability->>'is_open')::boolean, false) then
      raise exception 'Le créneau sélectionné ne correspond pas à une période d''ouverture du restaurant.';
    end if;
  else
    v_availability := public.get_restaurant_availability(v_restaurant.id);
    if not coalesce((v_availability->>'is_open')::boolean, false) then
      raise exception 'Ce restaurant est actuellement fermé. Vous pouvez programmer votre commande pour le prochain créneau disponible.';
    end if;
  end if;
  ...
  loop
    ...
    v_subtotal := v_subtotal + (v_product.price + v_options_price) * v_qty;
  ...
  if p_promo_code is not null and btrim(p_promo_code) <> '' then
    select * into v_promo_code_row
      from public.resolve_promo_code(v_restaurant.id, p_promo_code, v_promo_customer_id, greatest(v_subtotal - v_discount_amount, 0));

    if v_promo_code_row.error_message is not null then
      raise exception '%', v_promo_code_row.error_message;
    end if;

    v_promo_discount := coalesce(v_promo_code_row.discount_amount, 0);
    v_promo_waives_delivery := coalesce(v_promo_code_row.waives_delivery, false);
    v_promo_code_id_to_store := v_promo_code_row.promo_code_id;
    v_promo_code_snapshot := v_promo_code_row.code;
    v_discount_amount := v_discount_amount + v_promo_discount;
  end if;

  if v_settings.minimum_order is not null and (v_subtotal - v_discount_amount) < v_settings.minimum_order then
    raise exception 'Montant minimum de commande non atteint (% %)', v_settings.minimum_order, v_restaurant.currency;
  end if;

  if p_fulfillment_type = 'delivery' and not v_free_delivery and not v_promo_waives_delivery then
  ...
  v_pickup_code := case when p_fulfillment_type = 'delivery' then lpad(floor(random() * 100)::text, 2, '0') else null end;
  ...
  insert into public.orders (
    ...
    offer_id, offer_title_snapshot, promo_code_id, promo_code_snapshot, visitor_id,
    ...
    restaurant_lat_snapshot, restaurant_lng_snapshot, cutlery_requested, pickup_code, scheduled_for
  ) values (
    ...
    v_promo_code_id_to_store, v_promo_code_snapshot,
    ...
    v_restaurant.lat, v_restaurant.lng, coalesce(p_cutlery_requested, false), v_pickup_code, p_scheduled_for
  )
  returning id into v_order_id;

  if v_promo_code_id_to_store is not null then
    insert into public.promo_code_usages (promo_code_id, restaurant_id, customer_id, order_id, discount_amount)
    values (v_promo_code_id_to_store, v_restaurant.id, v_customer_id, v_order_id, v_promo_discount);
  end if;
  ...
  values (v_restaurant.id, v_order_id, null, 'pending', null, case when p_scheduled_for is not null then 'Commande programmée créée' else 'Commande créée' end);

  v_notif_body := format('Commande #%s · %s %s · %s article(s)', v_order_number, v_total, v_restaurant.currency, v_item_count);
  if p_scheduled_for is not null then
    v_notif_body := v_notif_body || format(' · Programmée pour %s', to_char(p_scheduled_for, 'DD/MM HH24:MI'));
  end if;
  ...
  perform public.create_notification(
    v_restaurant.id, v_order_id, 'new_order', case when p_scheduled_for is not null then 'Nouvelle commande programmée' else 'Nouvelle commande' end,
    v_notif_body,
    jsonb_build_object(
      ...
      'is_delivery', p_fulfillment_type = 'delivery', 'scheduled_for', p_scheduled_for
    )
  );
  ...
  return jsonb_build_object(
    ...
    'promo_code_id', v_promo_code_id_to_store,
