-- Second bug in the same "Impossible de créer la commande" report: after
-- fixing the v_offer record issue, create_order still failed with
-- "column phone_normalized does not exist". Its customer find-or-create
-- block was written against 20260828050000_customers_crm.sql's intended
-- schema (name, phone_normalized, first_seen_at/last_seen_at) -- but the
-- LIVE customers table has a different, already-in-use shape: full_name,
-- phone (holds the normalized value directly -- see lookup_customer_name,
-- which matches on `phone = normalize_phone(p_phone)`), and orders_count /
-- total_spent / first_order_at / last_order_at as running counters that
-- merge_customers re-syncs from real order aggregates. The admin Customers
-- panel already reads this exact live shape correctly (see
-- src/lib/customers-db.ts). Only create_order's customer block was out of
-- step with it -- fixed here, nothing else touched.
--
-- Note: this means 20260828050000_customers_crm.sql's own `create table if
-- not exists public.customers (...)` never actually created its table --
-- one already existed under this different shape -- so that migration's
-- table definition never took effect. Left as historical record rather
-- than rewritten; this migration corrects create_order to match reality.
