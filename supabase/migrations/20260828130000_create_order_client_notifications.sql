-- Additive p_visitor_id param so create_order can attribute the order to
-- the browser that placed it (client_notifications/orders.visitor_id) and
-- fire the "order_confirmed" client notification immediately, mirroring
-- the existing tenant-facing create_notification call right next to it.
-- CREATE OR REPLACE does not replace a function whose parameter list
-- changed -- it creates a second overload -- so the old 18-arg signature
-- is dropped explicitly at the end of this migration (same fix pattern as
-- the two prior duplicate-overload incidents this session).
...
drop function if exists public.create_order(text, order_fulfillment_type, text, text, jsonb, text, text, text, text, text, jsonb, text, double precision, double precision, text, text, text, uuid);
